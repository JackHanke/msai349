...
def cluster_alignment(query, means=kmeans):
...
def main(algorithm):
    ...
    if algorithm == 'knn':
        ...
        quant_metric = cluster_alignment(query=reduced_test_data, means=query_clusters)
        print(f'Cluster Alignment (entropy) is {quant_metric}')
